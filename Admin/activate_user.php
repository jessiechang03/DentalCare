...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
        ...
        $query = "UPDATE Login SET status='Active' WHERE fk_userid=?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param('s', $userID);

        if ($stmt->execute()) {
            echo "<script>alert('User account activated successfully!'); window.location.href = 'manage_user.php';</script>";
        } else {
            echo "<script>alert('Error activating user account.'); window.location.href = 'manage_user.php';</script>";
        }
...
<body>
    <div class="confirm-box">
        <h2>Are you sure you want to activate the account for <?php echo htmlspecialchars($userName); ?>?</h2>
